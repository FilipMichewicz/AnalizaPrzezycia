...
kable(raki1, 
      ...
      caption = "\\label{tab:raki1} Realizacje przedziałów przeżycia dla pacjentek z rakiem jajnika typu II i typu IIIA - Mayo Clinic - $\\tau = 1500$")
```

\hyperref[tab:raki1]{Tabela \ref*{tab:raki1}.}
...
kable(raki2, 
      ...
      row.names = TRUE,
      col.names = c("Grupa", "$T_L$", "$T_U$"),
      caption = "\\label{tab:raki2} Realizacje przedziałów przeżycia dla pacjentek z rakiem jajnika typu II i typu IIIA - Mayo Clinic - $\\tau = 2000$")
```

\hyperref[tab:raki2]{Tabela \ref*{tab:raki2}.}
...
Na początek łączymy poszczególne próby $\mathbb{X}_1,\dots,\mathbb{X}_k$, gdzie $\mathbb{X}_j = \left(X_{j1},\dots,X_{jn_j}\right)$, $j \in \{1,\dots,k\}$.  Z tak połączonych grup wyznaczamy (unikalne) czasy zdarzeń, których liczbę oznaczamy przed D, i porządkujemy je rosnąco: $t_1<t_2<\dots<t_D$.
...
$$
Z_j(\tau) = \sum^D_{i=1} W_j(t_i)\left(\frac{d_{ij}}{r_{ij}} - \frac{d_{i}}{r_{i}}\right), \ \text{gdzie:}
$$

* $d_{ij}$ - liczba zdarzeń w czasie $t_i$ z próbu $\mathbb{X}_j$,
* $r_{ij}$ - liczba jednostek w stanie ryzyka w czasie $t_i$ w j-tej grupie,
* $d_i = \sum^k_{j=1} d_{ij}$,
* $r_i = \sum^k_{j=1} r_{ij}$.

W zadaniu przyjęto funkcję wagi jako $W_j\left(t_i\right)=r_{ij}W\left(t_i\right)$, wtedy statystyka $Z_j(\tau)$ ma postać:

$$
Z_j(\tau) = \sum^D_{i=1} W(t_i)\left(d_{ij} - r_{ij}\frac{d_{i}}{r_{i}}\right).
$$

Wariancję takiej statystyki estymujemy przez:

$$
\hat{\sigma}_{jj} = \sum^D_{i=1}
...
$$
\hat{\sigma}_{jg} = - \sum^D_{i=1}
W^2(t_i)d_i\frac{r_{ij}r_{ig}}{r_i^2}\left(\frac{r_i - d_i}{r_i - 1}\right).
$$

Statystyka testowa do weryfikacji rozpatrywanych hipotez konstruowana jest w oparciu o dowolne k−1 wybranych statystyk $Z_j(\tau)$ i jest postaci:
...
$$
p = 1 - F_{\chi^2_{k - 1}} \left(z^2\right),
$$

gdzie $F_{\chi^2_{k - 1}}$ jest dystrybuantą rozkładu chi-kwadrat z k-1 stopniami swobody, a $z^2$ realizacją statystyki $Z^2$.

W zadaniu dokonano czterech testów ze względu na dobraną postać funkcji $W\left(t_i\right)$:

* test log-rank: $W\left(t_i\right) \equiv 1$,
* test Gehana-Breslowa: $W\left(t_i\right)=r_i$,
* test Peto-Peto: $W\left(t_i\right)=\prod_{t_j \leq t_i} \left(1-\frac{d_j}{r_j+1}\right)$,
* test Tarone'a-Were'a: $W\left(t_i\right)=\sqrt{r_i}$.
