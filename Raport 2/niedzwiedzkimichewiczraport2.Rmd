...
Jednakże wartość odchylenia standardowego $\sigma_n(t)$ nie jest znana. Można jednakże oszacować wariancję asymptotyczną $\sigma^2_n(t)$ przez formułę Greenwooda:
...
W powyższym przekształceniu składnik $\sqrt{n}$ został pominięty, gdyż ta "informacja zawiera się" w estymatorze $\hat{\sigma}_n(t)=\sqrt{\hat{\sigma}^2_n(t)}$. Na jego podstawie jesteśmy w stanie jednak wyznaczyć asymptotycznie punktowy przedział ufności dla wartości funkcji przeżycia $S$ w punkcie $t$ na poziomie ufności $1-\alpha$, który jest postaci:
...
Zadanie wyprowadzenia ścisłych własności asymptotycznych dla tego estymatora jest bardzo trudne, a autorzy wręcz sugerują, że może być niemożliwe do uzyskania w interesującej formie.
