...
# Kwantyle, generator z EW, cenzurowanie I typu, cenzurowanie losowe
# rozkładem wykładniczym i funkcja przeżycia rozkładu GE
...
cenzurowanie_losowe <- function(theta, data) {
  ...
  data.frame(times = censored_data, deltas = deltas)
}
...
EW_survival <- function(t, alpha, beta, gamm){
  1 - (1 - exp(-t/beta)^alpha)^gamm
}
```
...
points.A <- data.frame(
  time  = fit.KM.A$time[df.A$deltas == 0],
  surv  = fit.KM.A$surv[df.A$deltas == 0],
  group = "A"
)

points.B <- data.frame(
  time  = fit.KM.B$time[df.B$deltas == 0],
  surv  = fit.KM.B$surv[df.B$deltas == 0],
  group = "B"
)

points.KM.df <- rbind(points.A, points.B)
```
...
```{r KM-plot, echo=FALSE, fig.cap="Estymator Kaplana-Meiera dla danych dotyczących leków", warning=FALSE}
ggplot(plot.df, aes(x = time, y = surv, color = group)) +
  geom_step(linewidth = 1.2) +
  geom_point(data = points.KM.df,
...
\hyperref[fig:KM-plot]{Wykres \ref*{fig:KM-plot}.} przedstawia estymowaną funkcję przeżycia za pomocą estymatora Kaplana–Meiera. Dane w obu przypadkach są prawostronnie cenzurowane typu I, zatem największe wartości w zbiorach danych są cenzurowane, a po ostatnim pełnym (niecenzurowanym) czasie występują obserwacje cenzurowane, przez co estymator nie jest określony dla $t > t_{(n)}$. Dlatego do czasu $t \leq t_{(n)}$ estymator Kaplana–Meiera jest dobrze określony. W przypadku gdyby obserwacja $t_{(n)}$ była kompletna, to dla $t > t_{(n)}$  estymator wynosiłby $\hat{S}(t)=0$.
...
points.A <- data.frame(
  time  = fit.FH.A$time[df.A$deltas == 0],
  surv  = fit.FH.A$surv[df.A$deltas == 0],
  group = "A"
)

points.B <- data.frame(
  time  = fit.FH.B$time[df.B$deltas == 0],
  surv  = fit.FH.B$surv[df.B$deltas == 0],
  group = "B"
)

points.FH.df <- rbind(points.A, points.B)
```

Poniżej przedstawiono wykres estymowanej funkcji przeżycia uzyskanej przy użyciu estymatora Fleminga-Harringtona.

```{r FH-plot, echo=FALSE, fig.cap="Estymator Fleminga-Harringtona dla danych dotyczących leków", warning=FALSE}
ggplot(plot.df, aes(x = time, y = surv, color = group)) +
  geom_step(linewidth = 1.2) +
  geom_point(data = points.FH.df,
             aes(time, surv, color = group),
             shape = 3, size = 2) +
  scale_color_manual(values = c("A" = "steelblue", "B" = "tomato"),
                     labels = c("Lek A", "Lek B")) +
...
Obydwa estymatory dają podobne wyniki. W krótkim czasie lek A wydaje się korzystniejszy (niższa funkcja przeżycia, dłuższy czas do remisji), natomiast dla $t > 0,5$ bardziej efektywny staje się lek B. Do dokładniejszej analizy warto zastosować, np. test Kołmogorowa–Smirnowa.
...
Brown, Hollander i Kowar sugerowali oszacowanie ogona funkcji przeżycia przez odpowiednio dobraną funkcję przeżycia rozkładu wykładniczego, taką, aby w punkcie $t^+$ (ostatnim zarejestrowanym czasie) była równa $S(t^+)$.
...
```{r KM-ogon-plot, echo=FALSE, fig.cap="Estymator Kaplana-Meiera wraz z ogonem Browna, Hollandera i Kowara dla danych dotyczących leków", warning=FALSE}
...
$$
t_0 = \frac{\left( \psi(\alpha + 1) - \psi(1) \right)}{\lambda}
$$
...
```{r hist-1, echo=FALSE, fig.cap="Histogram oszacowań funkcji przeżycia w punktach $t_0$ oraz $2 t_0$ - liczność próby n = 30", warning=FALSE}
histograms$"30"
```
...
```{r hist-2, echo=FALSE, fig.cap="Histogram oszacowań funkcji przeżycia w punktach $t_0$ oraz $2 t_0$ - liczność próby n = 50", warning=FALSE}
histograms$"50"
```

\hyperref[fig:hist-2]{Wykres \ref*{fig:hist-2}.} przedstawia estymowaną wartość funkcji przeżycia w punktach $t_0$ oraz $2 t_0$ dla próby o liczności $n = 50$. W porównaniu z próbą o liczności $n = 30$ przedstawionej na \hyperref[fig:hist-1]{Wykresie \ref*{fig:hist-1}.} estymowane wartości przesunęły się ku mniejszym wartościom.

```{r hist-3, echo=FALSE, fig.cap="Histogram oszacowań funkcji przeżycia w punktach $t_0$ oraz $2 t_0$ - liczność próby n = 100", warning=FALSE}
histograms$"100"
```

\hyperref[fig:hist-3]{Wykres \ref*{fig:hist-3}.} przedstawia estymowaną wartość funkcji przeżycia w punktach $t_0$ oraz $2 t_0$ dla próby o liczności $n = 30$. Podobnie jak na poprzednim \hyperref[fig:hist-2]{Wykresie} wraz ze wzrostem próby wartości przesunęły się ku mniejszym wartościom. 
...
Z \hyperref[tab:testy-sw]{Tabeli \ref*{tab:testy-sw}.} można odczytać że dla wszystkich przypadków $p$-value jest równe zeru lub bliskie zera, a zatem dla prawie każdego poziomu $\alpha$ powinno się odrzucić hipotezę o normalności rozkładów, zarówno dla punktu $t_0$, jak i $2t_0$.
...
gdzie $\hat{S}(x)$ oznacza estymowaną funkcję przeżycia, a $\hat{\mu}$ - średni czas życia.

Ze względu na to że $\hat{S}(x)$ na przedziale $(0, t^+)$ jest funkcją schodkową, dlatego $\hat{\mu}$ będzie obliczana według wzoru:

$$
\hat{\mu} \phantom{cc} = \sum_{i: t_{(i)} \le t^+} \hat{S}\left(t_{(i-1)}\right)\left(t_{(i)} - t_{(i-1)}\right), \quad t_{(0)} := 0,
$$
...
Jeżeli natomiast estymator będzie zawierał "ogon" według propozycji Browna, Hollandera i Kowara należy powiększyć wartość $\hat{\mu}$ o wartość oczekiwaną na tym właśnie "ogonie".
...
Do wyznaczenia przedziałów ufności skorzystano z twierdzenia że, jeśli $F$ i $G$ są dystrybuantami odpowiednio czasu życia i czasu cenzurowania, ciągłymi na przedziale $[0,T]$, a ponadto $F(T) < 1$, to zachodzi zbieżność
...
$$
r_i = \sum_{j=1}^n \mathbf{1}_{[s_i,\infty)}(t_j)\qquad
d_i = \sum_{j=1}^n \mathbf{1}_{\{s_i\}}(t_j)\,\mathbf{1}_{\{1\}}(\delta_j),
$$
...
Na początek dokonano konkatenacji poszczególnych prób $\mathbb{X}_1,\dots,\mathbb{X}_k$, gdzie $\mathbb{X}_j = (X_{j1},\dots,X_{jn_j})$, $j \in \{1,\dots,k\}$. Z tak połączonych prób wyznaczono (unikalne) czasy zdarzeń, których liczbę oznaczono przez $D$, i uporządkowano je rosnąco: $t_1 < t_2 < \dots < t_D$.
...
Wariancję takiej statystyki estymuje się przez:
...
kowariancję natomiast estymuje się przez:
...
```{r testy-wykres, echo=FALSE, fig.cap="\\label{fig:udbzddp} Wagi testów nieparametrycznych dla porównania rozkładów czasów przeżycia - Mayo Clinic", warning=FALSE}
...
```{r Kaplan-Meier-rak, echo=FALSE, fig.cap="Estymator Kaplana-Meiera dla danych dotyczących pacjentek z rakiem jajnika - Mayo Clinic", warning=FALSE, warning=FALSE}
...
points.II <- data.frame(
  time  = fit.KM.II$time[df.II$deltas == 0],
  surv  = fit.KM.II$surv[df.II$deltas == 0],
  group = "II"
)

points.IIIA <- data.frame(
  time  = fit.KM.IIIA$time[df.III$deltas == 0],
  surv  = fit.KM.IIIA$surv[df.III$deltas == 0],
  group = "IIIA"
)

points.raki.df <- rbind(points.II, points.IIIA)

ggplot(plot.df, aes(x = time, y = surv, color = group)) +
  geom_step(linewidth = 1.2) +
    geom_point(data = points.raki.df,
             aes(time, surv, color = group),
             shape = 3, size = 2) +
...
Zadanie polega na dokonaniu symulacji porównujących asymptotyczne punktowo przedziały ufności, które oparte będą na własnościach asymptotycznych estymatora Kaplana-Meiera oraz metodzie delta. Rozważane będą trzy przekształcenia:

* liniowe: $g(x)=x$
* logarytmiczne: $g(x)=\ln(x)$
* logit: $g(x)=\ln\left(\frac{x}{1-x}\right)$

Jeżeli $F$ i $G$ są odpowiednio dystrybuantami czasu życia i czasu cenzury ciągłymi na przedziale $\left[0;T\right]$, oraz $F\left(T\right)<1$, to dla każdego $t \in \left(0,T\right)$:

$$
\frac{\hat{S}_n(t)-S(t)}{\sigma_n(t)} \xrightarrow{d} \mathcal{N}(0,1).
$$

Jednakże wartość odchylenia standardowego $\sigma_n(t)$ nie jest znana. Rozwiązaniem tego problemu jest oszacowanie asymptotycznej wariancji $\sigma^2_n(t)$ przez formułę Greenwooda:

\begin{equation}
\label{eq:greenwood1}
\hat{\sigma}^2_n(t)
  = \hat{S}^2(t)\sum_{i: t_{(i)} \le t}
    \frac{\delta_{(i)}}{(n-i+1)(n-i)}
\end{equation}


\begin{equation}
\label{eq:greenwood2}
\hat{\sigma}^2_n(t) = \hat{S}^2(t) \sum_{i: t_{(i)} \leq t} \frac{d_i}{r_i(r_i-d_i)}
\end{equation}

dla każdego $t<t_{(n)}$. Wzór (\ref{eq:greenwood1}) stanowi szczególny przypadek, gdy z prawdopodobieństwem 1 obserwacje się nie powtarzają, w (\ref{eq:greenwood2}) natomiast niektóre już mogą.

Metoda delta mówi nam, że jeżeli ciąg estymatorów $\left(X_n\right)_{n \ge 1}$ spełnia asymptotyczną normalność:

$$
\sqrt{n}\left(X_n-\vartheta\right) \xrightarrow{d} \mathcal{N}\left(0, \sigma^2\right),
$$

gdzie $\vartheta$ i $\sigma^2$ są pewnymi stałymi, oraz funkcja g jest różniczkowalna w punkcie $\vartheta$ i $g'\left(\vartheta\right)\neq0$, to ciąg $\left(g\left(X_n\right)\right)_{n\ge 1}$, spełnia:

$$
\sqrt{n}\left(g\left(X_n\right)-g\left(\vartheta\right)\right) \xrightarrow{d}
\mathcal{N}\left(0, \left[g'\left(\vartheta\right)\right]^2\sigma^2\right).
$$
W przypadku połączenia metody delta z własnościami asymptotycznymi estymatora Kaplana-Meiera, dochodzi się do następującego wniosku:

$$
\frac{g\left(\hat{S}_n(t)\right)-g\left(S(t)\right)}{g'\left(\hat{S}_n(t)\right)\hat{\sigma}_n(t)} \xrightarrow{d} \mathcal{N}(0,1).
$$

W powyższym przekształceniu składnik $\sqrt{n}$ został pominięty, gdyż ta "informacja zawiera się" w estymatorze $\hat{\sigma}_n(t)=\sqrt{\hat{\sigma}^2_n(t)}$. Na jego podstawie można wyznaczyć asymptotycznie punktowy przedział ufności dla wartości funkcji przeżycia $S$ w punkcie $t$ na poziomie ufności $1-\alpha$, który jest postaci:

$$
T_L = g^{-1}\left(g\left(\hat{S}_n(t)\right)-z\left(1-\frac{\alpha}{2}\right)g'\left(\hat{S}_n(t)\right)\hat{\sigma}_n(t)\right),
$$
$$
T_U = g^{-1}\left(g\left(\hat{S}_n(t)\right)+z\left(1-\frac{\alpha}{2}\right)g'\left(\hat{S}_n(t)\right)\hat{\sigma}_n(t)\right),
$$

gdzie $z(\cdot)$ jest funkcją kwantylową standardowego rozkładu normalnego.

Symulację przeprowadzono dla danych generowanych z rozkładu $\mathcal{G}\mathcal{E}(1, 2)$, losowo cenzurowanych zmiennymi z rozkładu $\mathcal{E}(0.7)$, przy $t=1.2$, dla próbek o liczności $n \in \left\{30,50,100\right\}$, wykonując 1000 powtórzeń przy poziomie istotności $\alpha=0.05$. Dla zwiększenia wiarygodności przedziały dla różnych przekształceń wyznaczano na tych samych danych.

Poniżej przedstawiono blok kodu w R realizujący symulację:

```{r symulacje-przedziały, echo=TRUE}
interval <- function(data, t, type="x", alpha=0.05){
  
  if(t >= max(data$times)) stop("t must be less than the
                                maximum time in the data")
  if(type == "x"){
    g <- function(x) return(x)
    g_inv <- function(y) return(y)
    g_der <- function(x) return(1)
  }
  else if(type == "log"){
    g <- function(x) return(log(x))
    g_inv <- function(y) return(exp(y))
    g_der <- function(x) return(1/x)
  }
  else if(type == "logit"){
    g <- function(x) return(log(x/(1-x)))
    g_inv <- function(y) return(1-1/(exp(y)+1))
    g_der <- function(x) return(1/(x*(1-x)))
  }
  else stop("Invalid option, choose one of the following:
            \"x\", \"log\", \"logit\"")
  
  fit <- survfit(Surv(times, deltas) ~ 1, data = data)
  fit <- data.frame(n.event = fit$n.event,
                    times = fit$time,
                    surv = fit$surv)
  fit_subset <- fit[fit$times <= t,] 
  sum <- 0
  
  if(length(unique(data$times)) != nrow(data)){
    for(time in fit_subset$time){
      r <- sum(data$times >= time)
      d <- sum(data$times == time & data$deltas == 1)
      sum <- sum + d/(r*(r-d))
    }
  }
  else{
    n <- nrow(fit)
    for(i in 1:nrow(fit_subset)){
      sum <- sum + fit_subset$n.event[i]/((n-i+1)*(n-i))
    }
  }
  
  St_estimation <- min(fit_subset$surv)
  variance <- sum * min(St_estimation)^2
  g_St_estimation <- g(St_estimation)
  g_der_St_estimation <- g_der(St_estimation)
  quantile <- qnorm(1-alpha/2)
  aux_variable <- quantile*g_der_St_estimation*sqrt(variance)
  
  TL <- g_St_estimation - aux_variable
  TU <- g_St_estimation + aux_variable
  
  return(c(g_inv(TL), g_inv(TU)))
}

# Symulacje
simulation <- function(theta, alpha1, lambd, t, n, M=1000, alpha=0.05){
  m <- 0
  length_x <- 0
  interval_x <- 0
  length_log <- 0
  ...
  length_logit <- 0
  interval_logit <- 0
  St <- EW_survival(t, alpha=1, beta=1/lambd, gamm=alpha1)
  
  while(m<M){
    generated_data <- GE_cenzurowanie_losowe(theta, alpha1, lambd, n)
    if(t >= max(generated_data$times)) next
    
    # type="x"
    est_interval <- interval(generated_data, t, type="x", alpha=alpha)
    lower <- est_interval[1]
    upper <- est_interval[2]
    if(lower <= St & St <= upper) interval_x <- interval_x+1
    length_x <- length_x + (upper-lower)
    
    # type="log"
    est_interval <- interval(generated_data, t, type="log", alpha=alpha)
    lower <- est_interval[1]
    upper <- est_interval[2]
    if(lower <= St & St <= upper) interval_log <- interval_log+1
    length_log <- length_log + (upper-lower)
    m <- m+1
    
    # type="logit"
    est_interval <- interval(generated_data, t, type="logit", alpha=alpha)
    lower <- est_interval[1]
    upper <- est_interval[2]
    if(lower <= St & St <= upper) interval_logit <- interval_logit+1
    length_logit <- length_logit + (upper-lower)
  }
  return(list("length" = c(length_x/M, length_log/M, length_logit/M),
              "interval" = c(interval_x/M, interval_log/M, interval_logit/M)))
}
```

```{r, symulacje-przedziały-wyniki}
df1 <- data.frame()
df2 <- data.frame()
number_of_observations <- c(30, 50, 100)
for(n in number_of_observations){
    wynik <- simulation(0.7, 2, 1, 1.2, n)
    df1 <- rbind(df1, wynik[[1]])
    df2 <- rbind(df2, wynik[[2]])
}

rownames(df1) <- as.character(number_of_observations)
rownames(df2) <- as.character(number_of_observations)
```

```{r symulacje-przedziały-długość, echo=FALSE}
kable(df1, digits=3, escape=FALSE,
      col.names = c("n", "$g(x)=x$", "$g(x)=\\ln(x)$", "$g(x)=\\ln(\\frac{x}{1-x})$"),
      caption="\\label{tab:symulacje-długość} Porównanie asymptotycznych punktowo przedziałów ufności funkcji przeżycia - długość przedziału")
```

Wyniki przedstawione w \hyperref[tab:symulacje-długość]{Tabeli \ref*{tab:symulacje-długość}.} pokazują, że długość przedziału ufności maleje wraz ze wzrostem liczności próby. Świadczy to o rosnącej precyzji estymacji funkcji przeżycia metodą Kaplana–Meiera. Dla dowolnej próby o długości $n$ najszersze przedziały uzyskano przy zastosowaniu przekształcenia logarytmicznego, natomiast najwęższe - po zastosowaniu transformacji logit.

```{r symulacje-przedziały-prawdopodobienstwo, echo=FALSE}
kable(df2, digits=3, escape=FALSE,
      col.names = c("n", "$g(x)=x$", "$g(x)=\\ln(x)$", "$g(x)=\\ln(\\frac{x}{1-x})$"),
      caption="\\label{tab:symulacje-prawd} Porównanie asymptotycznych punktowo przedziałów ufności funkcji przeżycia - prawdopodobieństwo pokrycia")
```

Wyniki przedstawione w \hyperref[tab:symulacje-prawd]{Tabeli \ref*{tab:symulacje-prawd}.} pokazują, że wraz ze wzrostem liczności próby prawdopodobieństwo pokrycia może zarówno rosnąć, jak i maleć, co świadczy o stabilizacji wyników.  
Na poziomie ufności 95% przekształcenie logit wykazuje niewielkie przeszacowanie, natomiast transformacje liniowa i logarytmiczna - niedoszacowanie. Można jednak przypuszczać, że wraz ze wzrostem liczby prób wyniki zbliżą się do zadanego poziomu ufności. Podobnie jak wcześniej, transformacja logit pozostaje najlepszym wyborem.

Podsumowując wyniki z obu tabel, najbardziej korzystną transformacją jest funkcja logit. Wybór między brakiem przekształcenia a przekształceniem logarytmicznym zależy od priorytetu: krótszego przedziału ufności czy większego prawdopodobieństwa pokrycia.

\newpage
## Zadanie 2

Estymatory Kaplana-Meiera oraz Fleminga–Harringtona są funkcjami schodkowymi. W stosunkowo małych próbach często obserwuje się szerokie przedziały czasu, w których estymator jest stały - praktycznie i interpretacyjnie bywa to niepożądane i statystykom trudno jest wytłumaczyć praktykom na przykładzie \hyperref[fig:KM-plot]{Wykresu \ref*{fig:KM-plot}.} grupy leku A dlaczego prawdopodobieństwo przeżycia do chwili $t = 0.7$ jest równe prawdopodobieństwu przeżycia do chwili $t = 0.8$.
...
Dla dowolnego $t\ge 0$ definiuje się wektor wag
$$
...
Rozróżnia się trzy przypadki dla różych $m$:
...
B) Gdy $m$ jest nieparzyste tzn. $m=2k+1$ dla pewnego $k\in\mathbb{N}$. Wagi definiuje się następująco:

W pierwszej kolejności zdefiniowano indeks $j$ opisujący, w którym przedziale znajduje się $t$:
...
1. Jeżeli $j \le k$ (t blisko początku) - krawędź lewa:
...
$$
2. Jeżeli $j\ge N-k$ (t blisko końca) - krawędź prawa:
...
3. Jeżeli $k< j < N-k$ ($t$ jest dostatecznie daleko od krawędzi i otoczenie o rozmiarze $m$ mieści się w zakresie indeksów) - przypadek środkowy:
    ...
    1. Blok jednostkowy (dokładnie $m$ indeksów)
       Dla indeksów $i\in{j-k,j-k+1,\dots,j+k}$ przyjmuje się $w_i(t)=1.$
...
C) Gdy $m$ jest parzyste tzn. $m=2k$ dla pewnego $k\in\mathbb{N_+}$. Wagi definiuje się następująco:
...
w_{m+1}(t)=\dots=w_N(t)=0.
$$
2. Jeżeli $j\ge N-k+1$ (t blisko końca) - krawędź prawa:
...
3. Jeżeli $k< j < N-k+1$ ($t$ jest dostatecznie daleko od krawędzi i otoczenie o rozmiarze $m$ mieści się w zakresie indeksów) - przypadek środkowy:
    ...
    1. Blok jednostkowy (dokładnie $m$ indeksów)
       Dla indeksów $i\in{j-k+1,j-k+2,\dots,j+k}$ przyjmuje się $w_i(t)=1.$
...
W pierwszej kolejności obliczono log-czasy zdarzeń oraz ich log-log przeżycia, które następnie stanowią dane wejściowe do lokalnej regresji liniowej budującej estymator Rossy-Zielińskiego.

$$
X_j = \log T_{(j)}, \qquad
Y_j = \log\left(-\log(KM'_j)\right),
$$
W przypadku $m=2$ i w ustalonej chwili $t$ obliczono log-t:
...
\begin{cases}
Y_{1}
+\dfrac{Y_{2}-Y_{1}}{\,X_{2}-X_{1}\,}(X - X_{1}),& 0 < t \le T_{(1)},\\
Y_{j}+\dfrac{Y_{j+1}-Y_{j}}{\,X_{j+1}-X_{j}\,}(X - X_{j}),& T_{(j)} < t \le T_{(j+1)},\\
Y_{N-1}+\dfrac{Y_{N}-Y_{N-1}}{\,X_{N}-X_{N-1}\,}(X - X_{N-1}),& t > T_{(N)} \ \text{ i } \ \delta_{N}=1,\\
\text{ nieokreślony }, & t > T_{(N)} \ \text{ i } \ \delta_{N}=0.
\end{cases}
$$

W przypadku $m>2$ i w ustalonej chwili $t$, dysponuje się wektorem wag:
...
które interpretuje się jako lokalne parametry kształtu i skali rozkładu Weibulla dopasowanego w "otoczeniu" punktu $t$ przy użyciu wag opisanych wcześniej.
...
W przypadku $m>2$ rekonstruuje się lokalną funkcję przeżycia, korzystając z postaci funkcji przeżycia rozkładu Weibulla:
...
Porównano graficznie estymator Kaplana-Maiera z jego ulepszoną wersją: estymatorem Rossy-Zielińskiego.
...
rz_km_plot <- function(df, m1 = 2, m2 = 5, to = 3) {
  ...
  df_km1 <- data.frame(time = c(0, fit$time), surv = c(1, fit$surv),
                       group = "KM", panel = "M2")
  df_km2 <- data.frame(time = c(0, fit$time), surv = c(1, fit$surv),
                       group = "KM", panel = "M5")
  
  points.km1 <- data.frame(
    time  = fit$time[df$deltas == 0],
    surv  = fit$surv[df$deltas == 0],
    group = "KM",
    panel = "M2"
  )
  
  points.km2 <- data.frame(
    time  = fit$time[df$deltas == 0],
    surv  = fit$surv[df$deltas == 0],
    group = "KM",
    panel = "M5"
  ...
  points_df <- rbind(points.km1, points.km2)
  
  ggplot() +
    geom_step(data = subset(plot_df, group == "KM"),
              aes(x = time, y = surv, color = group),
              linewidth = 0.8) +
    geom_point(data = points_df,
               aes(x = time, y = surv, color = group),
               shape = 3, size = 2) +
    geom_line(data = subset(plot_df, group == "RZ"),
              aes(x = time, y = surv, color = group),
              linewidth = 0.8) +
    ...
    scale_color_manual(values = c("KM" = "tomato", "RZ" = "steelblue"),
                       labels = c("KM" = "Kaplana-Meiera",
                                  "RZ" = "Rossy-Zielińskiego")) +
    scale_y_continuous(limits = c(0, 1)) +
...
```{r rossa-zielinski1, echo = FALSE, fig.cap="Porównanie estymatora Kaplana-Meiera oraz Rossy-Zielińskiego dla zbioru danych klinicznych pacjentów", warning=FALSE}
...
```{r rossa-zielinski2, echo = FALSE, fig.cap="Porównanie estymatora Kaplana-Meiera oraz Rossy-Zielińskiego dla zbioru danych dotyczących leków - lek A", warning=FALSE}
...
```{r rossa-zielinski3, echo = FALSE, fig.cap="Porównanie estymatora Kaplana-Meiera oraz Rossy-Zielińskiego dla zbioru danych dotyczących leków - lek B", warning=FALSE}
...
```{r rossa-zielinski4, echo = FALSE, fig.cap="Porównanie estymatora Kaplana-Meiera oraz Rossy-Zielińskiego dla zbioru danych dotyczących pacjentów z rakiem jajnka - typ II", warning=FALSE}
...
```{r rossa-zielinski5, echo = FALSE, fig.cap="Porównanie estymatora Kaplana-Meiera oraz Rossy-Zielińskiego dla zbioru danych dotyczących pacjentów z rakiem jajnka - typ IIIA", warning=FALSE}
...
Zadanie wyprowadzenia ścisłych własności asymptotycznych dla tego estymatora jest bardzo trudne, a autorzy \cite{rossa2002} wręcz sugerują, że może być niemożliwe do uzyskania w interesującej formie. Jednak twierdzą oni, że dla dużej liczności próby $n$, tym samym dla dużej liczby $N$, ich estymator będzie zachowywał się jak estymator Kaplana-Meiera, tym samym zachowując jego asymptotyczne własności. Informacja ta znajduje się na końcu trzeciej sekcji, strona 10.
...
```{r adrian, echo=FALSE}
# Hej Adrian jak to czytasz to hej.
```