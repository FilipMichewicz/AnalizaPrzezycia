...
Zadanie polega na dokonaniu symulacji porównujących asymptotyczne punktowo przedziały ufności, które oparte będą na własnościach asymptotycznych estymatora Kaplana-Meiera oraz metodzie delta. Rozważanę będą trzy przekształcenia:
...
$$
1) \ \hat{\sigma}^2_n(t) = \hat{S}^2(t) \sum_{i: t_{(i)} \leq t} \frac{\delta_{(i)}}{(n-i+1)(n-i)} \\
2) \ \hat{\sigma}^2_n(t) = \hat{S}^2(t) \sum_{i: t_{(i)} \leq t} \frac{d_i}{r_i(r_i-d_i)}
$$

dla każdego $t<t_{(n)}$. Wzór 1) stanowi szczególny przypadek, gdy z prawdopodobieństwem 1 obserwacje się nie powtarzają, w 2) natomiast niektóre już mogą.

Metoda delta mówi nam, że jeżeli ciąg $\left(X_n\right)$, $n\geq1$, zmiennych losowych spełnia:

$$
\sqrt{n}\left(X_n-\vartheta\right) \xrightarrow{d} \mathcal{N}\left(0, \sigma^2\right),
$$

gdzie $\vartheta$ i $\sigma^2$ są pewnymi stałymi, oraz funkcja g jest różniczkowalna w punkcie $\vartheta$ i $g'\left(\vartheta\right)\neq0$, to ciąg $\left(g\left(X_n\right)\right)$, $n\geq1$ jest taki, że:
...
gdzie $z$ jest funkcją kwantylową standardowego rozkładu normalnego.

Symulacja została przeprowadzona dla danych generowanych z rozkładu $\mathcal{G}\mathcal{E}(1, 2)$, cenzurowanych losowo za pomocą zmiennych losowych z rozkładu $\mathcal{E}(0.7)$, dla $t=1.2$, liczności próbek $n \in \left\{30,50,100\right\}$, ilości powtórzeń równej 1000 i $\alpha=0.05$. Dla dodatkowej wiarygodności, przedziały dla poszczególnych przekształceń były wyliczane na tych samych danych. Poniżej przedstawiony jest kod realizujący tę symulację.
...
Wyniki z \hyperref[tab:symulacje-długość]{tabeli \ref*{tab:symulacje-długość}.} wskazują skracanie się przedziału wraz z licznością próby. To z kolei świadczy o rosnącej precyzji estymacji funkcji przeżycia metodą Kaplana-Meiera. Dla dowolnej próby długości n najszersze przedziały otrzymujemy za pomocą przekształcenia logarytmicznego, najwęższe zaś po zastosowaniu transformacji logit.
...
\hyperref[tab:symulacje-prawd]{Tabela \ref*{tab:symulacje-prawd}.} pokazują, że wraz ze wzrostem liczebności próby prawdopodobieństwo pokrycia może maleć lub rosnąć. Oznacza to stabilizację wyników. Na poziomie ufności 95% przekształcenie logit wykazuje lekkie przeszacowanie, natomiast liniowe i logarytmiczne - niedoszacowanie. Można jednak przypuszczać, że wraz z większą liczbą próbek wyniki zbliżą się do zadanego poziomu ufności. Podobnie jak wcześniej transformacja logit jest najlepszym wyborem.

Podsumowując wyniki z obu tabel, najlepszą transformacją okazuje się funkcja logit. Wybór między brakiem przekształcenia a przekształceniem logarytmicznym zależy od tego, na czym bardziej komu zależy - odpowiednio na krótszym przedziale, czy na większym prawdopodobieństwie pokrycia.
