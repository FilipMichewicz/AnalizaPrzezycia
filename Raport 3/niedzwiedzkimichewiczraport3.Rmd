...
\exp\left(\left(\frac{\exp\left(\beta^Tz_2\right)x}{\lambda}\right)^k -
\left(\frac{\exp\left(\beta^Tz_1\right)x}{\lambda}\right)^k \right)
$$
...
W takim razie prawdopodobieństwo, że osoba A starsza o rok od osoby B przeżyje czas $x$ jest równe szansie, że osoba B przeżyje czas o $0.9\%$ dłuższy ($1.009x$). Starszy wiek zwiększa w każdej chwili szansę na zgon.
...
Ze względu na uzyskane pokrycie, w dalszej części raportu wyniki będą porównywane jedynie z modelem PH, gdyż wnioski dla modelu AFT będą identyczne.
...
knitr::kable(
  df.table,
  col.names = c("", "PH", "Cox"),
  caption = "\\label{tab:porownanie}Estymowana funkcja przeżycia dla czasu t = 300 w procentach - porównanie modeli PH oraz Coxa",
  digits = 2
)
```

Porównując prawdopodobieństwa z \hyperref[tab:porownanie]{Tabeli  \ref*{tab:porownanie}.}, można dojść do wniosku, że estymowane prawdopodobieństwa wartości dla modeli PH i Cox są do siebie bardzo zbliżone. To może sugerować, że nawet mimo braku założenia konkretnego rozkładu w modelu Coxa, wyniki są zbliżone do tych uzyskanych przy ustalonym z góry rozkładzie Weibulla.

## Zadanie 6

W tym zadaniu narysowano wykresy estymowanej funkcji przeżycia dla 70-letniej kobiety o stopniu sprawności fizycznej ECOG równym 1 oraz stopniu Karnofsky równym 90 dla modeli PH oraz Coxa.

```{r gomboc1, fig.cap="Porównanie estymowanych funkcji przeżycia - modele PH oraz Coxa"}
t <- seq(0, max(df_surv_z1$time), length.out = 10000)
df <- data.frame(t = t,
                 s = surv_fun_ph(t, z1))

ggplot() +
  geom_line(data = df, aes(x = t, y = s, color = "PH"), lwd = 0.7) +
  geom_line(data = df_surv_z1, aes(x = time, y = survival, color = "Cox"), lwd = 0.7) +
  scale_color_manual(
    name = "Model",
    values = c("PH" = "steelblue", "Cox" = "tomato")
  ) +
  labs(x = "Czas do zgonu (t)", y = "S(t|z)") +
  theme_minimal() +
  theme(legend.position = "bottom")
...
\hyperref[fig:gomboc1]{Wykres \ref*{fig:gomboc1}.} wskazuje zbliżoną do siebie estymację prawdopodobieństw uzyskaną modelami Coxa i PH. To z kolei sugeruje, że założenie dotyczące rozkładu Weibulla było prawidłowe.
...
$$
\frac{\theta_{z_1}(t)}{\theta_{z_2}(t)} = \exp\left(\beta^T(z_2-z_1)\right), \\
\phantom{} \\
\ln \frac{\theta_{z_1}(t)}{\theta_{z_2}(t)} = \beta^T(z_2-z_1),
$$
...
kable(
  ...
  digits = 3,
  caption = "\\label{tab:odds_wspolczynniki} Współczynniki modelu proporcjonalnych szans"
)
```

## Zadanie 2

W tym zadaniu zinterpretowane zostały wyznaczone współczynniki dla poszczególnych zmiennych znajdujące się w \hyperref[tab:odds_wspolczynniki]{tabeli \ref*{tab:odds_wspolczynniki}.} Interpretacja opiera się na ilorazie estymowanych szans i dla jednostki o charakterystyce $z_i$ przesuniętej o jeden punkt
względem średniej próbki wynosi $\exp\left(\beta_i\right)$. Jest to równoznaczne z tym, że szansa tej jednostki na zdarzenie przed chwila $t$ jest równe szansie jednostki o zerowym wektorze charakterystyk pomnożonej przez wartość $\exp\left(\beta_i\right)$, niezależnie od ustalonego $t$.
...
W przypadku wieku każdy dodatkowy rok życia zwiększa szansę na zgon o około 1,3% ($\exp(0.0132) \approx 1.0132$).

Zmiana skali ECOG z wartości 0 na wartość 1 powoduje wzrost szansy na śmierć o prawie 73% ($\exp(0.547) \approx 1.728$). W przypadku wartości ECOG = 2 szansa ta wzrasta ponad czterokrotnie ($\exp(1.4466) \approx 4.2487$), a ECOG = 3: prawie siedmiokrotnie ($\exp(1.4466) \approx 4.2487$).

Wzrost skali Karnofsky'ego powoduje zmniejszenie szansy śmierci o wartość poniżej 0.4% ($1-\exp(-0.0037) \approx  0.369$).

Szansa mężczyzn na śmierć przed chwilą $t$ jest około 2.5 raza większa niż w przypadku kobiet ($\exp(0.9535) \approx 2.595$).

## Zadanie 3

Zadanie polega na oszacowaniu wartości skumulowanej funkcji hazardu oraz przeżycia. W tym celu zastosowano funkcję `predict` z pakietu `stats`. Wykorzystano zależność $H(t)=-\log\left(S(t)\right)$.

```{r odds_bazowe, echo=TRUE}
S_0 <- predict(model.odds, Z=rep(0, 6))
S <- data.frame("time"=S_0$time,
                "S0"=c(S_0$S0),
                "H0"=c(-log(S_0$S0)))
```

```{r odds_bazowa_przeżycia, fig.cap="Estymowane wartości bazowej funkcji przeżycia - model proporcjonalnych szans", eval=FALSE}
ggplot(S, aes(x=time)) +
  geom_line(aes(y=H0), lwd = 0.7, color="tomato") +
  labs(x="Czas do zgonu (t)", y= expression(hat(H)(t)))
#\hyperref[fig:odds_bazowa_przeżycia]{Wykres \ref*{fig:odds_bazowa_przeżycia}.}
```

```{r odds_bazowa_skum, fig.cap="Estymowane wartości bazowej skumulowanej funkcji hazardu - model proporcjonalnych szans", eval=FALSE}
ggplot(S, aes(x=time)) +
  geom_line(aes(y=S0), lwd = 0.7, color="tomato") +
  labs(x="Czas do zgonu (t)", y= expression(hat(S)(t)))
#\hyperref[fig:odds_bazowa_skum]{Wykres \ref*{fig:odds_bazowa_skum}.}
```
...
surv_cum_haz_fun_odds <- function(z) {
  ...
  df <- data.frame("time" = S$time,
                   "S" = 1 / (1 + (1-S$S0)/S$S0 * theta_odd))
  df$H <- -log(df$S)
  df$odds <- (1-df$S)/df$S
  df
}
...
```{r hazardplot4, fig.cap="Estymowana skumulowana funkcja hazardu dla odpowiednich charakterystyk 70-letnich kobiet - modele Coxa i proporcjonalnych szans"}
...
ggplot() +
  geom_line(aes(x=time, y=H, color="Odds; 1"), Z1, lwd=0.7) +
  geom_line(aes(x=time, y=H, color="Odds; 2"), Z2, lwd=0.7) +
...
\hyperref[fig:hazardplot4]{Wykres \ref*{fig:hazardplot4}.} pokazuje, że dla modelu proporcjonalnych szans estymowane wartości funkcji skumulowanego hazardu są mniejsze, niż te dla modelu PH Coxa. Dla kobiety o niższym stopniu ECOG = 1 różnica ta wygląda na stałą i niewielką. Jednakże przy ECOG = 2 różnica ta jest dużo bardziej wyraźna i rosnąca w czasie. Wyniki takie oznaczają, że w każdej chwili $t$ model PO wskazuje mniejsze ryzyko zgonu niż model PH Coxa, tym samym szacowane prawdopodbieństwa przeżycia bedą większe.
...
```{r hazardplot5, fig.cap="Logarytm estymowanej skumulowanej funkcji hazardu dla odpowiednich charakterystyk 70-letnich kobiet - model proporcjonalnych szans"}
Z1_log <- Z1[Z1$H>0,]
...
ggplot() +
  ...
  labs(x = "Czas do zgonu (t)", y = "log(H(x|z))") +
  theme_minimal() +
  theme(legend.position = "bottom")
```

Z \hyperref[fig:hazardplot5]{wykresu \ref*{fig:hazardplot5}.} można wyciągnąć przypuszczenia, że różnica logarytmów skumulowanych funkcji hazardu (odpowiednio: iloraz skumulowanych funkcji hazardu) nie jest stały w czasie i zależy od wartości $t$.

```{r hazardplot6, fig.cap="Różnica logarytmów estymowanej skumulowanej funkcji hazardu dla odpowiednich charakterystyk 70-letnich kobiet - model proporcjonalnych szans"}
...
\hyperref[fig:hazardplot6]{Wykres \ref*{fig:hazardplot6}.} potwierdza nasze poprzednie wnioski. Należy jednak pamiętać, że model proporcjonalnych szans, jak sama nazwa wskazuje, zachowuje stałą wartość ilorazu estymowanych funkcji szans, a nie skumulowanych funkcji hazardu. Model ten stosuje się, gdy drugi przypadek nie zachodzi.

```{r oddsplot, fig.cap="Różnica logarytmów estymowanej szansy dla odpowiednich charakterystyk 70-letnich kobiet"}
df <- data.frame("time"=Z1_log$time,
                 "difference"=log(Z1_log$odds)-log(Z2_log$odds))
ggplot(df, aes(x=time)) +
  geom_line(aes(y=difference), df, lwd=0.7, color="tomato") +
  labs(x = "Czas do zgonu (t)", y = expression("Różnica logarytmów " * hat(theta)(t))) +
  theme_minimal() +
  theme(legend.position = "bottom")
```

Na podstawie \hyperref[fig:oddsplot]{wykresu \ref*{fig:oddsplot}.} nie ma podstaw do wątpienia w poprawność przyjętego modelu proporcjonalnych szans. Widoczna różnica logarytmów funkcji szans pozostaje stała w całym badanym przedziale czasu, tym samym nie zależy od jego wartości.

## Zadanie 5

Celem zadania jest oszacować dla wcześniej wymienionych pacjentek prawdopodobieństwo, że ich czas życia będzie większy niż 300 dni, na podstawie modelu proporcjonalnych szasn. Dodatkowo dokonano porównania otrzymanych wyników z tymi uzyskanymi metodą proporcjonalnych hazardów Coxa.
...
knitr::kable(
  df.table,
  col.names = c("", "Cox", "Odds"),
  caption = "\\label{tab:porownanie_odds}Estymowana funkcja przeżycia dla czasu t = 300 w procentach - porównanie modeli Coxa oraz proporcjonalnych szans",
  digits = 2
)
```

Porównanie wyników z \hyperref[tab:porownanie_odds]{tabeli \ref*{tab:porownanie_odds}.} prowadzi do wniosku, że estymowane wartości dla modelu PO są większe (korzystniejsze dla pacjentek) niż te dla modelu Coxa. Potwierdza to również wniosek wysnuty na podstawie \hyperref[fig:hazardplot4]{wykresu \ref*{fig:hazardplot4}.} - mniejszy skumulowany hazard oznacza mniejsze ryzyko, a mniejsza różnica między danymi funkcjami daje również mniejszą różnicę w szacowanych prawdopodobieeństwach.

## Zadanie 6

W zadaniu narysowano wykres estymowanej funkcji przeżycia dla pacjentki o stopniu skali ECOG = 1 dla modelu proporcjonalnych szans oraz dokonano graficznego porównania z funkcją uzyskaną za pomocą modelu PO Coxa.

```{r gomboc2, fig.cap="Porównanie estymowanych funkcji przeżycia - modele Coxa oraz proporcjonalnych szans"}
...
ggplot() +
  geom_line(data = df_odds, aes(x = time, y = S, color = "Odds"), lwd = 0.7) +
  geom_line(data = df_cox, aes(x = time, y = survival, color = "Cox"), lwd = 0.7) +
  scale_color_manual(
    name = "Model",
    values = c("Odds" = "steelblue", Cox = "tomato")
  ) +
...
\hyperref[fig:gomboc2]{Wykres \ref*{fig:gomboc2}.} pokazuje, że estymowane wartości prawdopodobieństwa są większe dla modelu PO niż Coxa o około $10\%$. Należy jednak zauważyć, że nie zmienia się ona gwałtownie wraz z czasem, lecz stopniowo.

Biorąc pod uwagę wyniki uzyskane dla wszystkich analizowanych podejść można przypuszczać, że duże rozbieżności dla modelu proporcjonalnych szans mogą wynikać ze względu na nieadekwatnego dopasowania do danych. Faktycznie, model ten stosuje się, gdy nie jest spełnione założenie proporcjonalności hazardów, które w przypadku naszych danych jednak zostało potwierdzone empirycznie.
