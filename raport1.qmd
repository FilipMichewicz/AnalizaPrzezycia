...
Lista pierwsza obejmuje analizę wykøadniczego rozkładu Weibulla $\mathcal{EW} \left( \alpha, \beta, \gamma \right)$; definicje jego funkcji, generowanie danych, wizualizację oraz porównanie statystyk empirycznych i teoretycznych.
...
```{python}
#| echo: false
#| fig-cap: "Histogram i gęstość teoretyczna dla próby z rozkładu $\\mathcal{EW} \\left( \\alpha = 1, \\beta = 2, \\gamma = 1 \\right)$ - liczność prób n = 50 oraz n = 100 \\label{fig:hist1}"
#| fig-align: center
...
```{python}
#| echo: false
#| fig-cap: "Histogram i gęstość teoretyczna dla próby z rozkładu $\\mathcal{EW} \\left( \\alpha = \\tfrac{1}{2}, \\beta = 1, \\gamma = 4 \\right)$ - liczność prób n = 50 oraz n = 100 \\label{fig:hist2}"
#| fig-align: center
...
Nie istnieją wzory analityczne na wyliczenie tych całek dla wykładniczego rozkładu Weibulla, zatem otrzymane wartości porównano porównywać z wbudowaną funkcją *scipy.integrate.quad*. Dla jak najlepszej dokładności wyników, przedziałem całkowania będzie $\left[0; Q(0.999)\right]$, gdzie $Q\left(\cdot\right)$ - funkcja kwantylowa. Całka została policzona metodą Simpsona.
...
Tabela przedstawia wyniki uzyskane metodą parabol, metodą Monte Carlo oraz przy użyciu funkcji `scipy.integrate.quad`. Dla weryfikacji poprawności obliczeń można dobrać parametry tak, aby rozkład przyjmował postać klasycznego wykładniczego lub rozszerzonego wykładniczego, a następnie porównać wartości obliczone numerycznie z wynikami analitycznymi.
...
W \hyperref[tab:adam1]{Tabeli \ref{tab:adam1}.} i \hyperref[tab:adam2]{Tabeli \ref{tab:adam2}.} przedstawiono wartości oczekiwane oraz wariancje rozkładu $\mathcal{EW}(\alpha, \beta, \gamma)$ dla wybranych parametrów; widać, że wszystkie zastosowane metody numeryczne dają bardzo zbliżone wyniki, co potwierdza ich poprawność i stabilność estymacji.
...
W \hyperref[tab:beata2]{Tabeli \ref{tab:beata2}.} widać że grupy stosujące lek A oraz lek B nie różnią się znacząco między  Jednak jeżeli mamy już porównywać ich skuteczność, to lek B wypada lepiej. Zarówno mediana, jak i kwartyl dolny posiadają mniejsze wartości. Jest to równoznaczne z tym, że średni czas leczenia lekiem B jest krótszy niż lekiem A. Na korzyść leku A natomiast wypada (choć nieznacznie) minimum: pierwsza wyleczona osoba korzystała właśnie z niego. Natomiast liczba nieskutecznych prób (cenzurowanych) jest równa w obu przypadkach.
...
**UWAGA**: Poniższe fragmenty kodu pozwalają na wyznaczenie estymatorów punktowych oraz przedziałów ufności dla próbek zmiennych losowych pochodzących z rozkładu wykładniczego postaci $f(x)=\vartheta\exp\left(-\vartheta x\right)\mathbb{1}_{\left(0;\infty\right)}(x)$, dla którego wartość oczekiwana wynosi $\mathbb{E}\left[X\right]=\frac{1}{\vartheta}$. W zadaniach 1. oraz 2. chcemy otrzymać średni czas do remisji choroby, czyli $\mathbb{E}\left[X\right]=\mu$. Z tego wynika, że $\vartheta=\frac{1}{\mu}$. Z tego powodu wyniki będą podawane w postaci:
...
Poniżej przedstawiono kod w języku Python służący do wyznaczania estymatora największej wiarogodności dla danych cenzurowanych I typu z rozkładu wykładniczego $\mathcal{E}(\vartheta)$. Dodatkowo dokonano estymacji parametrycznej parametru $\vartheta$ dla danych z Listy 2. Zadania 3. za pomocą metody największej wiarogodności.
...
```{python}
#| echo: false
#| tbl-cap: "Przedziały ufności dla danych cenzurowanych II typu - różne poziomy ufności \\label{tab:cedrik2}"
...
Z \hyperref[tab:cedrik2]{Tabeli \ref{tab:cedrik2}.} można odczytać, że powstałe przedziały ufności są węższe dla danych dotyczących leku A.
...
n_values = [10, 30]
t0_values = [0.5, 1, 2]
n_sim = 10000 # PRZED WYSŁANIEM ZMIEŃ NA 10000 (tak jest w tekście powyżej) i ten komentarz skasuj
...
Warto zauważyć, że wraz ze wzrostem liczebności próby różnice w obciążeniu i MSE między estymatorami stają się coraz mniejsze.
...
W następnej kolejność zbadano hipoteże zerową $H_0: \ \vartheta = \vartheta_0 = 2$, przy hipotezie alternatywnej $H_1: \ \vartheta \neq \vartheta_0$ dla 10 alternatyw $\vartheta_0 \in \{1.1, 1.3, 1.5, 1.7, 1.9, 2.0, 2.1, 2.3, 2.5, 2.7\}$. Dla każdego przypadku wykonano 10 000 replikacji (które było powtarzane w przypadku $R=0$). Wyniki przedstawiono w tabeli poniżej.
...
\hyperref[tab:bartosz]{Tabeli \ref{tab:bartosz}.} bla bla bla.
...
Z \hyperref[tab:tomek]{Tabeli \ref{tab:tomek}.} odczytujemy, że wartości p są większe od przyjętego poziomu istotności, zatem nie ma podstaw do odrzucenia hipotezy zerowej $H_0$.
